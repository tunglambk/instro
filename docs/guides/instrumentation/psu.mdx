...
| `get_voltage_setpoint(channel=N)` | Read the configured voltage setpoint in volts (only regulates the output while in CV mode) |
| `get_current_setpoint(channel=N)` | Read the configured current-limit setpoint in amperes (only regulates the output while in CC mode) |
...
<Note>
`get_voltage_setpoint()` and `get_current_setpoint()` return the configured target the supply is programmed to reach, not the measured output (`get_voltage()`/`get_current()`).
CV/CC mode only identifies which setpoint is currently regulating the output. `get_voltage_setpoint()` regulates the output while the supply is in constant-voltage (CV) mode, and `get_current_setpoint()` regulates the output while it's in constant-current (CC) mode.
</Note>
