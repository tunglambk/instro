...
| `get_voltage_setpoint(channel=N)` | Read the configured voltage setpoint in volts (may differ from actual voltage output outside of constant voltage mode) |
| `get_current_setpoint(channel=N)` | Read the configured current-limit setpoint in amperes (may differ from actual current-limit output outside of constant current mode) |
...
<Note>
Both the voltage setpoint and the current-limit setpoint stay programmed and readable regardless of mode. Constant Voltage (CV)/Constant Current (CC) mode determines which setpoint currently regulates the output. 
The actual measured voltage may differ from the voltage setpoint when not in CV mode. Similarly, the actual measured current may differ from the current-limit setpoint when not in CC mode.
</Note>
