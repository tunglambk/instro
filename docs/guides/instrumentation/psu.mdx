...
A supply can't activate independent voltage and current setpoints at the same time. `get_voltage_setpoint()` reflects the voltage target while the supply is in constant-voltage (CV) mode, and `get_current_setpoint()` reflects the current target while it's in constant-current (CC) mode. 
</Note>
