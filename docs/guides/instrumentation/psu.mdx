...
def get_voltage_setpoint(self, channel: int) -> float:
    """Query the voltage setpoint (volts) on `channel`; only regulates the output in constant-voltage mode."""

def get_current_setpoint(self, channel: int) -> float:
    """Query the current-limit setpoint (amperes) on `channel`; only regulates the output in constant-current mode."""
