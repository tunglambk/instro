...
| `get_voltage_setpoint(channel=N)` | Read the configured voltage setpoint in volts (measured output may differ outside CV mode) |
| `get_current_setpoint(channel=N)` | Read the configured current-limit setpoint in amperes (measured output may differ outside CC mode) |
...
<Note>
Both the voltage setpoint and the current-limit setpoint stay programmed and readable regardless of mode. CV/CC mode determines which setpoint currently regulates the output. The measured output can differ from the setpoint that isn't in control.
</Note>
