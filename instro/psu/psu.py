...
class PSUDriverBase(abc.ABC):
    ...
    def get_voltage_setpoint(self, channel: int) -> float:
        """Query the configured voltage setpoint (volts) on `channel`; only regulates the output in constant voltage mode."""
    ...
    def get_current_setpoint(self, channel: int) -> float:
        """Query the configured current-limit setpoint (amperes) on `channel`; only regulates the output in constant current mode."""
...
class InstroPSU(Instrument):
    ...
    def get_voltage_setpoint(self, channel: int, **kwargs) -> Measurement | None:
        """Query the voltage setpoint (volts) on ``channel``; only regulates the output in CV mode. Returns ``None`` if unavailable."""
        return self._execute_measurement(
    ...
    def get_current_setpoint(self, channel: int, **kwargs) -> Measurement | None:
        """Query the current-limit setpoint (amperes) on ``channel``; only regulates the output in CC mode. Returns ``None`` if unavailable."""
        return self._execute_measurement(
